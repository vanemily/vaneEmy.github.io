...
Código fuente de [vanemy.me](https://vanemy.me), sitio personal de Vanessa Emily. Es un sitio [Jekyll](https://jekyllrb.com/) servido con GitHub Pages.
...
La primera vez, si el script se queja de que faltan gemas:
...
(El sistema trae un Ruby viejo — por eso la versión de bundler fijada. El `Gemfile` ya pinea versiones compatibles.)
...
assets/js/main.js         # todo el JS del sitio (guardado por elemento: si no existe, no hace nada)
...
1. Crea el archivo en `_drafts/nombre-lo-que-sea.md` con al menos:
   ```yaml
   ---
   title: "Título de la memoria"
   ---
   ```
2. Escribe el cuerpo en Markdown normal.
...
No hay ambiente de staging: `git push origin master` publica directo en vanemy.me (GitHub Pages reconstruye con Jekyll automáticamente, 1-2 minutos).
